...
class ConstructionViewNode : SKNode {
    ...
    init(bottomBar_height: CGFloat, bottomBar_buffer: CGFloat) {
        ...
        for i in 0..<constructionStructures.count {
            constructionStructures[i].building.size = iconSize
            constructionStructures[i].building.position = point
            constructionStructures[i].building.anchorPoint = anchorPoint
            
            point.x += iconSize.width + xBuffer
            
            addChild(constructionStructures[i].building)
        }
    }
    ...
    func startStructureCreation(name: String, location: CGPoint) {
        ...
        if name.contains("constructor") {
            if name.contains("reactor") {
                ...
                structure = Structure(texture: Structures.reactorLevel1)
            }
        }
        
        structure.isDisabled = true
        structure.position = location
        structure.name?.append("_underConstruction")
