...
extension GameScene {
    ...
    func endConstructionMode() {
        if isValidSpot {
            ...
            // Mineral Cost
            PlayerManager.shared.players[toBuild!.playerID]?.minerals_spend(amount: toBuild!.constructionCost)
...
//            }
    ...
    func searchStructuresForPlayerInRange(playerID: Int, isSupplier: Bool) -> [Structure] {
        ...
        let playerStructures = EntityManager.shared.entitiesForPlayer[playerID]!
        for target in playerStructures {
            // If the entity is not a structure, skip it
            if let type = target.component(ofType: EntityTypeComponent.self),
                type.entityType != .structure {
                continue
            }
            
            // Cast it to a structure
            let targetStructure = target as! Structure
